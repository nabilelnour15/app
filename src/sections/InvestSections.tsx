    ...
    switch (activeTab) {
          ...
          >
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {investData.publicFreeZone.businesses.map((biz, idx) => (
                <div
                  key={idx}
                  className="p-6 bg-gradient-to-br from-white/10 to-transparent rounded-2xl border border-white/5 flex items-center gap-4"
                >
          ...
          >
            ...
            <div className="grid lg:grid-cols-3 gap-8">
              {investData.investmentDrivers.map((driver, idx) => (
                <div
                  key={idx}
                  className="bg-white/5 p-8 rounded-3xl border border-white/10 relative overflow-hidden group"
                >
                  ...
                  <div className="mb-6">
                    <PlaceholderImage
                      text={`${driver.title} Visual`}
                      src={driver.image}
                      alt={`${driver.title} Visual`}
                  ...
                  {driver.types && (
                    <div className="flex flex-wrap gap-2">
                      {driver.types.map((t) => (
                        <span
                          key={t}
                          className="px-3 py-1 bg-[#3898ec]/10 rounded-full text-xs text-[#3898ec] border border-[#3898ec]/20"
                        >
                          {t}
                        </span>
                      ))}
          ...
          >
            ...
            <div className="grid md:grid-cols-1 gap-8">
              {investData.rawMaterials.map((mat, idx) => (
                <div
                  key={idx}
                  className="bg-gradient-to-r from-white/5 to-transparent p-8 rounded-3xl border border-white/10"
                >
                  <div className="flex flex-col lg:flex-row gap-8">
                    <div className="flex-1">
                      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                        <div>
                          <h4 className="text-2xl font-bold text-[#3898ec]">
                            {mat.name}
                          </h4>
                        ...
                        {mat.reserve && (
                          <div className="px-5 py-2 bg-white/5 rounded-2xl border border-white/10">
                            <span className="text-white/40 text-xs block uppercase">
                              Reserve
                            </span>
                      ...
                      {mat.uses && (
                        <div className="mt-4 p-4 bg-[#3898ec]/5 rounded-xl border border-[#3898ec]/10">
                          <span className="text-[#3898ec] font-bold text-sm block mb-1">
                            Applications
                          </span>
                    ...
                    <div className="lg:w-1/3">
                      <PlaceholderImage
                        text={`${mat.name} extraction`}
                        src={mat.image}
                        alt={`${mat.name} extraction`}
      ...
      case "ports":
        return (
          ...
          >
            <h3 className="text-3xl font-bold text-white mb-6 font-['Montserrat']">
              Global Gateway Ports
            </h3>
            <div className="grid md:grid-cols-2 gap-8">
              {investData.ports.map((port, idx) => (
                <div
                  key={idx}
                  className="group relative bg-[#1a2b41] p-8 rounded-3xl border border-white/10 overflow-hidden"
                >
                  <div className="mb-6">
                    <PlaceholderImage
                      text={`${port.name} View`}
            ...
            <div className="grid lg:grid-cols-2 gap-12 items-center">
              <div>
                <h3 className="text-3xl font-bold text-white mb-8 font-['Montserrat']">
                  Legal Protections & Incentives
                </h3>
                <div className="grid gap-4">
                  {investData.investmentLaws.provisions.map((law, idx) => (
                    <div
                      key={idx}
                      className="flex gap-4 p-5 bg-white/5 rounded-2xl border border-white/5"
                    >
              ...
              <PlaceholderImage
                text="Legal & Regulatory Framework"
            ...
            <div className="bg-gradient-to-br from-[#0068c8] to-[#3898ec] p-10 rounded-[2.5rem] relative overflow-hidden">
              <div className="relative z-10">
                <h4 className="text-2xl font-bold text-white mb-6">
                  Investment Sectors Governed
                </h4>
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {investData.investmentLaws.fields.map((field) => (
                    <div
                      key={field}
                      className="px-4 py-3 bg-white/10 backdrop-blur-md rounded-xl text-white/90 text-sm border border-white/10"
                    >
                      {field}
                    </div>
                  ))}
          ...
          <aside className="lg:w-1/4">
            <nav className="sticky top-32 space-y-2">
              {categories.map((cat) => (
                ...
                >
                  <cat.icon
                    className={`w-5 h-5 ${activeTab === cat.id ? "text-white" : "group-hover:text-[#3898ec] transition-colors"}`}
                  />
